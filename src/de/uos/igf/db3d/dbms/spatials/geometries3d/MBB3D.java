...
public class MBB3D implements Geometry3D, Externalizable {
	...
	public MBB3D(Point3D pMin, Point3D pMax) {
		...
		if (!testMaxPoint(pMax)) {
			throw new IllegalArgumentException(
					Db3dSimpleResourceBundle.getString("db3d.geom.pmaxpmin"));
		}
	...
	public double computeVolume() {
		double volume = 1;
		volume *= (getPMax().x - getPMin().x);
		volume *= (getPMax().y - getPMin().y);
		volume *= (getPMax().z - getPMin().z);
		return volume;
	}
	 ...
	 * 
	 * @see MBB3D#containsStrict(MBB3D, GeoEpsilon)
	 * @param mbb
	 *            MBB3D for containment
	 * @return boolean - true if intersects, false otherwise.
	 */
	public boolean contains(MBB3D mbb, GeoEpsilon sop) {
		for (int i = 0; i < 3; i++) {
	 ...
	 * 
	 * @see MBB3D#contains(MBB3D, GeoEpsilon)
	 * @param mbb
	 *            MBB3D for containment
	 * @return boolean - true if intersects, false otherwise.
	 */
	public boolean containsStrict(MBB3D mbb, GeoEpsilon sop) {
		for (int i = 0; i < 3; i++) {
	 ...
	 */
	public boolean contains(Point3D point, GeoEpsilon sop) { // Dag
		for (int i = 0; i < 3; i++) {
	 ...
	 */
	public boolean containsStrict(Point3D point, GeoEpsilon sop) { // Dag
		for (int i = 0; i < 3; i++) {
			if (sop.greaterOrEqual(getPMin().getCoord(i), point.getCoord(i))
					|| sop.lessOrEqual(getPMax().getCoord(i), point.getCoord(i)))
				return false;
		}
		return true;
	}
	 ...
	 */
	public Set<Segment3D> getSegments(GeoEpsilon sop) { // Dag
	 ...
	 *             - if the index of the point coordinate is not 0, 1 or 2.
	 */
	public Rectangle3D[] getFaces(GeoEpsilon sop) { // Dag
		...
		if (sop.equal(pt[0].x, pt[1].x)) {
			for (int i = 0; i < 4; i++)
				pts[i] = new Point3D(pt[0]);
			pts[1].setCoord(1, pt[1].y);
			pts[2].setCoord(1, pt[1].y);
			pts[2].setCoord(2, pt[1].z);
			pts[3].setCoord(2, pt[1].z);
			return new Rectangle3D[] { new Rectangle3D(pts[0], pts[1], pts[2],
					pts[3]) };
		}
		if (sop.equal(pt[0].y, pt[1].y)) {
			for (int i = 0; i < 4; i++)
				pts[i] = new Point3D(pt[0]);
			pts[1].setCoord(1, pt[1].x);
			pts[2].setCoord(1, pt[1].x);
			pts[2].setCoord(2, pt[1].z);
			pts[3].setCoord(2, pt[1].z);
			return new Rectangle3D[] { new Rectangle3D(pts[0], pts[1], pts[2],
					pts[3]) };
		}
		if (sop.equal(pt[0].z, pt[1].z)) {
			for (int i = 0; i < 4; i++)
				pts[i] = new Point3D(pt[0]);
			pts[1].setCoord(1, pt[1].x);
			pts[2].setCoord(1, pt[1].x);
			pts[2].setCoord(2, pt[1].y);
			pts[3].setCoord(2, pt[1].y);
			return new Rectangle3D[] { new Rectangle3D(pts[0], pts[1], pts[2],
					pts[3]) };
		}
	...
	public Point3D getCenter() {
		double x = ((getPMax().x - getPMin().x) / 2) + getPMin().x;
		double y = ((getPMax().y - getPMin().y) / 2) + getPMin().y;
		double z = ((getPMax().z - getPMin().z) / 2) + getPMin().z;
	 ...
	 * 
	 * @return MBB3D always.
	 * @see Spatial.dbms.geom.SimpleGeoObj#getGeometryType()
	 */
	public GEOMETRYTYPES getGeometryType() {
		return GEOMETRYTYPES.MBB;
	}
	 ...
	 * 
	 * @see MBB3D#insideStrict(MBB3D, GeoEpsilon)
	 * @param mbb
	 *            MBB3D for inclusion
	 * @return boolean - true if intersects, false otherwise.
	 */
	public boolean inside(MBB3D mbb, GeoEpsilon sop) {
		for (int i = 0; i < 3; i++) {
	 ...
	 * 
	 * @see MBB3D#inside(MBB3D, GeoEpsilon)
	 * @param mbb
	 *            - MBB3D for inclusion
	 * @return boolean - true if intersects, false otherwise.
	 */
	public boolean insideStrict(MBB3D mbb, GeoEpsilon sop) {
		for (int i = 0; i < 3; i++) {
	 ...
	 * 
	 * @see MBB3D#intersectsStrict(MBB3D, GeoEpsilon)
	 * @param mbb
	 *            MBB3D for intersection
	 * @return boolean - true if intersects, false otherwise.
	 */
	public boolean intersects(MBB3D mbb, GeoEpsilon sop) {
		for (int i = 0; i < 3; i++) {
			if ((sop.greaterThan(getPMin().getCoord(i),
					mbb.getPMax().getCoord(i)))
	 ...
	 * 
	 * @see MBB3D#intersects(MBB3D, GeoEpsilon)
	 * @param mbb
	 *            MBB3D for intersection
	 * @return boolean - true if intersects, false otherwise.
	 */
	public boolean intersectsStrict(MBB3D mbb, GeoEpsilon sop) {
		for (int i = 0; i < 3; i++) {
	 ...
	 *             point is not greater than its minimum point.
	 */
	public MBB3D intersection(MBB3D mbb, GeoEpsilon sop) {
	 ...
	 *             the class Line3D (which computes the intersection of two
	 ...
	 *             Vector3D.
	 */
	public Geometry3D intersection(Line3D line, GeoEpsilon sop) { // Dag
		...
		for (int i = 0; i < 2; i++) {
			for (int j = 0; j < 3; j++) {
				...
				Geometry3D obj = line.intersection(plane, sop);
				if (obj != null) {
					if (obj.getGeometryType() == GEOMETRYTYPES.LINE) { // obj ==
																		// line
	 ...
	 *             Vector3D.
	 */
	public Geometry3D intersection(Plane3D plane, GeoEpsilon sop) { // Dag

		Rectangle3D[] face = this.getFaces(sop);
		WireframeGeometry3D resultWF = new WireframeGeometry3D(sop);
		...
		for (int i = 0; i < length; i++) {
			...
			Geometry3D obj = face[i].intersection(plane, sop);
			...
			if (obj != null) {
				...
				case SEGMENT:
					...
					break;
				case WIREFRAME:
					return ((WireframeGeometry3D) obj);
				}
			}
		}
	 ...
	 */
	public double minDist(Point3D point, GeoEpsilon sop) {
	 ...
	 * @return double - minimal distance.
	 */
	public double minDistSquare(Point3D point, GeoEpsilon sop) {
		double min = 0.0;
		double ri = 0.0;
		...
		for (int i = 0; i < 3; i++) {
			if (sop.lessThan(point.getCoord(i), this.getPMin().getCoord(i)))
				...
				if (sop.greaterThan(point.getCoord(i),
						this.getPMax().getCoord(i)))
	 ...
	 *             point is not greater than its minimum point.
	 */
	public MBB3D union(MBB3D mbb, GeoEpsilon sop) {
	 ...
	 * @return MBB3D - this.
	 * 
	 * @see Spatial.dbms.structure.GeoObj#getMBB()
	 */
	public MBB3D getMBB() {
		return this;
	}
	 ...
	 */
	public boolean isEqual(MBB3D obj, GeoEpsilon sop) {
		if (obj == null)
			return false;
		if (this == obj)
			return true;
		if (!pMax.isEqual(obj.pMax, sop))
			return false;
		if (!pMin.isEqual(obj.pMin, sop))
			return false;
		return true;
	}
	...
	public boolean isEqual(Equivalentable obj, GeoEpsilon sop) {
		...
		if (this == obj)
			return true;
		if (!(obj instanceof MBB3D))
			return false;
		return isEqual((MBB3D) obj, sop);
	}
	...
	/**
	 * Checks whether this MBB3D is equal to the given MBB3D. Strong typed test
	 * method because point-index of MBB3Ds have a natural order.
	 * 
	 * @param obj
	 *            MBB3D for test
	 * @return boolean - true if equal.
	 */
	public boolean isGeometryEquivalent(MBB3D obj, GeoEpsilon sop) {
		if (obj == null)
			return false;
		if (this == obj)
			return true;
	...
	public boolean isGeometryEquivalent(Equivalentable obj, GeoEpsilon sop) {
		...
		if (!(obj instanceof MBB3D))
			return false;
		return isGeometryEquivalent((MBB3D) obj, sop);
	}
