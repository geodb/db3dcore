 ...
 */
public class Rectangle3D implements Geometry3D {
	...
	public Segment3D getSegment(int index) {
		if (this.getPoint(index).equals(this.getPoint((index + 1) % 4)))
			return null;
		return new Segment3D(this.getPoint(index),
				this.getPoint((index + 1) % 4), null);
	}
	 ...
	 *             Vector3D.
	 */
	public Geometry3D intersectionInPlane(Line3D line, GeoEpsilon sop) {
	...
	public Geometry3D intersection(Plane3D plane, GeoEpsilon sop) {
		...
		for (int i = 0; i < length; i++) {
			...
			Geometry3D obj = seg[i].intersection(plane, sop);
			if (obj != null) {
				if (obj.getGeometryType() == GEOMETRYTYPES.POINT)
					pointHS.add(obj);
				if (obj.getGeometryType() == GEOMETRYTYPES.SEGMENT) {
					segCounter++;
					pointHS.add(seg[i].points[0]);
					pointHS.add(seg[i].points[1]);
					if (segCounter == 2) {
	...
	public GEOMETRYTYPES getGeometryType() {
		return GEOMETRYTYPES.RECTANGLE;
	}

	@Override
	public de.uos.igf.db3d.dbms.spatials.geometries3d.MBB3D getMBB() {
