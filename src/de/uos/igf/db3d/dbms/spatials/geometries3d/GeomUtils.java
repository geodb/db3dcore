...
import de.uos.igf.db3d.dbms.spatials.geometries3d.LA.SingularException;
...
public final class GeomUtils {
	...
	public static int doubleCompare(double d1, double d2) {
		...
		throw new IllegalArgumentException(
				Db3dSimpleResourceBundle.getString("db3d.geom.comparenan"));
	...
	static double affineDistance(double[][] a, double[][] b, double[] coeffsA,
			double[] coeffsB, GeoEpsilon scOp) {
		...
		while (systemHasChanged) {
			...
			for (int i = 0; i < inactiveAlength; ++i) {
				currentVec = LA.plus(currentVec,
						LA.mul(+kktX[aStart + i], a[piA[i]]));
			}
			for (int j = 0; j < inactiveBlength; ++j) {
				currentVec = LA.plus(currentVec,
						LA.mul(-kktX[bStart - j], b[piB[j]]));
			}
	...
	public static double simplexDistance(double[][] a, double[][] b,
			double[] coeffsA, double[] coeffsB, GeoEpsilon scOp) {
	...
	public static int simplexVertexRelation(double[][] simplex,
			double[] vertex, double[] coeffs, GeoEpsilon scOp, boolean projected) {
	...
	public final static MBB3D getExtendedMBB(Tetrahedron3D tetra, double psi) {
		Point3D pMin = new Point3D(GeomUtils.getMin(tetra.points[0].x,
				tetra.points[1].x, tetra.points[2].x, tetra.points[3].x) - psi,
				GeomUtils.getMin(tetra.points[0].y, tetra.points[1].y,
						tetra.points[2].y, tetra.points[3].y) - psi,
				GeomUtils.getMin(tetra.points[0].z, tetra.points[1].z,
						tetra.points[2].z, tetra.points[3].z) - psi);
		Point3D pMax = new Point3D(GeomUtils.getMax(tetra.points[0].x,
				tetra.points[1].x, tetra.points[2].x, tetra.points[3].x) + psi,
				GeomUtils.getMax(tetra.points[0].y, tetra.points[1].y,
						tetra.points[2].y, tetra.points[3].y) + psi,
				GeomUtils.getMax(tetra.points[0].z, tetra.points[1].z,
						tetra.points[2].z, tetra.points[3].z) + psi);
	 ...
	 *             point is not greater than its minimum point.
	 */
	public final static MBB3D getExtendedMBB(Triangle3D triangle, double psi) {
		Point3D pMin = new Point3D(GeomUtils.getMin(triangle.points[0].x,
				triangle.points[1].x, triangle.points[2].x) - psi,
				GeomUtils.getMin(triangle.points[0].y, triangle.points[1].y,
						triangle.points[2].y) - psi, GeomUtils.getMin(
						triangle.points[0].z, triangle.points[1].z,
						triangle.points[2].z)
						- psi);
		Point3D pMax = new Point3D(GeomUtils.getMax(triangle.points[0].x,
				triangle.points[1].x, triangle.points[2].x) + psi,
				GeomUtils.getMax(triangle.points[0].y, triangle.points[1].y,
						triangle.points[2].y) + psi, GeomUtils.getMax(
						triangle.points[0].z, triangle.points[1].z,
						triangle.points[2].z)
						+ psi);
		return new MBB3D(pMin, pMax);
	 ...
	 *             point is not greater than its minimum point.
	 */
	public final static MBB3D getExtendedMBB(Segment3D segment, double psi) {
		return new MBB3D(new Point3D(GeomUtils.getMin(segment.points[0].x,
				segment.points[1].x) - psi, GeomUtils.getMin(
				segment.points[0].y, segment.points[1].y) - psi,
				GeomUtils.getMin(segment.points[0].z, segment.points[1].z)
						- psi), new Point3D(GeomUtils.getMax(
				segment.points[0].x, segment.points[1].x) + psi,
				GeomUtils.getMax(segment.points[0].y, segment.points[1].y)
						+ psi, GeomUtils.getMax(segment.points[0].z,
						segment.points[1].z) + psi));
	}
	...
	public final static MBB3D getExtendedMBB(Point3D point, double psi) {
		return new MBB3D(new Point3D(point.x - psi, point.y - psi, point.z
				- psi),
				new Point3D(point.x + psi, point.y + psi, point.z + psi));
	}
	...
	private static final class Point3D_X_Comparator implements
		...
		public int compare(Object o1, Object o2) {
			return doubleCompare(((Point3D) o1).x, ((Point3D) o2).x);
		}
	}
	...
	private static final class Point3D_Y_Comparator implements
		 ...
		 */
		public int compare(Object o1, Object o2) {
			return doubleCompare(((Point3D) o1).y, ((Point3D) o2).y);
		}
	}
	...
	private static final class Point3D_Z_Comparator implements
		 ...
		 */
		public int compare(Object o1, Object o2) {
			return doubleCompare(((Point3D) o1).z, ((Point3D) o2).z);
		}
	}
	...
	private static final class Point3D_XYZ_Comparator implements
		...
		public int compare(Object o1, Object o2) {
			...
			result = doubleCompare(p1.x, p2.x);
			if (result != 0) {
				return result;
			}

			result = doubleCompare(p1.y, p2.y);
			if (result != 0) {
				return result;
			}

			return doubleCompare(p1.z, p2.z);
		}
	}
