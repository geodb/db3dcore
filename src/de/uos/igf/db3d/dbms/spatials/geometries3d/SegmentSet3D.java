 ...
 */
public class SegmentSet3D implements Geometry3D {
	 ...
	 */
	public SegmentSet3D(GeoEpsilon sop, int constant) {
	 ...
	 *            int constant for SegmentSet3D
	 */
	public SegmentSet3D(int initialCapacity, GeoEpsilon sop, int constant) {
	 ...
	 *            int constant for SegmentSet3D
	 */
	public SegmentSet3D(Set initialSet, GeoEpsilon sop, int constant) {
	 ...
	 * @see Spatial.dbms.geom.GeoObject#getGeometryType()
	 */
	public GEOMETRYTYPES getGeometryType() {
		return GEOMETRYTYPES.SEGMENTSET;
	}
	...
	public MBB3D getMBB() {
		if (size() > 0) {
			...
			double xMin = seg.points[0].x;
			double yMin = seg.points[0].y;
			double zMin = seg.points[0].z;
			if (xMin > seg.points[1].x)
				xMin = seg.points[1].x;
			if (xMin > seg.points[1].y)
				xMin = seg.points[1].y;
			if (xMin > seg.points[1].z)
				xMin = seg.points[1].z;
			double xMax = xMin;
			double yMax = yMin;
			double zMax = zMin;
			...
			while (it.hasNext()) {
				...
				for (int i = 0; i < 2; i++) {
					...
					value = seg.points[i].x;
					if (xMin > value)
						xMin = value;
					else if (xMax < value)
						xMax = value;

					value = seg.points[i].y;
					if (yMin > value)
						yMin = value;
					else if (yMax < value)
						yMax = value;

					value = seg.points[i].z;
					if (zMin > value)
						zMin = value;
					else if (zMax < value)
						zMax = value;
