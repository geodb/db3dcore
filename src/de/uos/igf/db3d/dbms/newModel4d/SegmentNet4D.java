...
/**
 * This class represents a 4D SegmentNet. A SegmentNet objects consists of one
 * to many SegmentComponent objects.
 * 
 ...
 */
public class SegmentNet4D implements Net4D {
	...
	Map<Integer, SegmentComponent4D> components;
	...
	/**
	 * Constructor for a SegmentNet4D. The initial start date is set. Call
	 * createEndOfExistenceInterval() function to set an end date.
	 * 
	 * @param start
	 *            - Start Date
	 */
	public SegmentNet4D(Date start) {
		super();
		components = new HashMap<Integer, SegmentComponent4D>();
	 ...
	 * 
	 * @param SegmentComponent4D
	 */
	public void addSegmentComponent(SegmentComponent4D component) {
	 ...
	 *         this net.
	 */
	public Map<Integer, SegmentComponent4D> getComponents() {
		return components;
	}
	 ...
	 * @return Component4D
	 */
	public SegmentComponent4D getComponent(int ID) {
		return components.get(ID);
	}

	/**
	 * Add a single Segment to the net.
	 * 
	 * @param Segment
	 */
	public void addSegment(Segment4D segment) {
		...
		if (elements.get(elements.size()-1).containsKey(segment.getID())) {
			throw new IllegalArgumentException(
					"You tried to add a triangle that already exists to the TriangleNet.");
		}
		elements.get(elements.size()-1).put(segment.getID(), segment);
	}
