 ...
 */
public class SegmentNet3DBuilder {
	
    public static void main(String[] args) {
        SegmentNet3DBuilder snb = new SegmentNet3DBuilder(new ScalarOperator());

        SegmentNet3DElement[] segArr = new SegmentNet3DElement[2];
        segArr[0] = new SegmentNet3DElement(new Point3D(1.0, 1.0, 1.0), new Point3D(
                      2.0, 1.0, 1.0), new ScalarOperator());
        segArr[1] = new SegmentNet3DElement(new Point3D(2.0, 1.0, 1.0), new Point3D(
                      3.0, 1.0, 1.0), new ScalarOperator());

        snb.addComponent(segArr);

        SegmentNet3DElement segElt = (SegmentNet3DElement) snb.getSegmentNet()
	 ...
	 */
	public SegmentNet3DBuilder(Space3DC space) {
	 ...
	 */
	public SegmentNet3DBuilder(ScalarOperator sop) {
		this.compIDCounter = 0;
		this.space = null;
	 ...
	 */
	public void addComponent(SegmentNet3DElement[] elements) {
		SegmentNet3DComponent comp = new SegmentNet3DComponent(
	 ...
	 *             getPoint(int) of the class Tetrahedron3D.
	 */
	public void addComponent(SegmentNet3DElement[] elements, int id) {
		SegmentNet3DComponent comp = new SegmentNet3DComponent(space.getScalarOperator()
				.copy(), elements);
		// Here an IllegalArgumentException can be thrown.
	...
	public SegmentNet3D getSegmentNet() {
		...
		SegmentNet3DComponent[] compnet = new SegmentNet3DComponent[components.size()];
		for (int i = 0; i < compnet.length; i++)
			compnet[i] = (SegmentNet3DComponent) components.get(i);
