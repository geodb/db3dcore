...
import de.uos.igf.db3d.dbms.model3d.api.Sample3D;
...
public class PointNet3D extends SpatialObject3D implements Sample3D,
	...
	/* components of this */
	private PointNet3DComponent[] components;
	 ...
	 */
	protected PointNet3D(PointNet3DComponent[] components, ScalarOperator sop) {
		super();
	...
	public PointNet3D(PointNet3D net) {
		super();
		PointNet3DComponent[] comps = net.getComponents();
		this.components = new PointNet3DComponent[comps.length];
	 ...
	 */
	public PointNet3DComponent[] getComponents() {
	 ...
	 */
	public PointNet3DComponent getComponent(int index) {
	 ...
	 */
	public void addComponent(PointNet3DComponent comp) {
		// set the element ids for this net
		Iterator it = comp.getElementsViaSAM().iterator();
		while (it.hasNext())
			((PointNet3DElement) it.next()).setID(this.nextElementID());

		// set component id
		comp.setComponentID(this.nextComponentID());

		// copy component
		PointNet3DComponent[] comps = getComponents();
		int compsLength = 0;
		if (comps != null)
			compsLength = comps.length;
		PointNet3DComponent[] temp = new PointNet3DComponent[compsLength + 1];
	 ...
	 */
	public void removeComponent(PointNet3DComponent comp) {
		PointNet3DComponent[] comps = getComponents();
		PointNet3DComponent[] temp = new PointNet3DComponent[comps.length - 1];
		int x = 0;
		for (int i = 0; i < comps.length; i++) {
			if (comps[i] != comp) {
				temp[x] = comps[i];
	 ...
	 */
	public PointNet3DComponent createComponent() {
		PointNet3DComponent comp = new PointNet3DComponent(getScalarOperator().copy());
		addComponent(comp);
		return comp;
	}
	...
	public PointNet3D splitPointNet(int[] indexes) { // Dag
		PointNet3DComponent[] newPointNetComps = new PointNet3DComponent[indexes.length];
	...
	public int countVertices() {
		int count = 0;
		PointNet3DComponent[] comps = this.getComponents();
		for (int i = 0; i < comps.length; i++)
	 ...
	 */
	public byte getType() {
		return ComplexGeoObject.POINT_NET_3D;
	}
	 ...
	 *            PointNet3DComp[]
	 */
	protected void setComponents(PointNet3DComponent[] comps) {
	...
	public SimpleGeoObj getElement(int id) {
		for (PointNet3DComponent comp : components) {
