...
import de.uos.igf.db3d.dbms.model3d.api.Surface3D;
...
public class TriangleNet3D extends SpatialObject3D implements Surface3D,
	...
	/** components of this */
	protected TriangleNet3DComponent[] components;
	 ...
	 */
	protected TriangleNet3D(TriangleNet3DComponent[] components, ScalarOperator sop) {
		super();
	...
	public TriangleNet3D(TriangleNet3D net) {
		super();
		TriangleNet3DComponent[] comps = net.getComponents();
		this.components = new TriangleNet3DComponent[comps.length];
	 ...
	 */
	public TriangleNet3DComponent[] getComponents() {
	 ...
	 */
	public TriangleNet3DComponent getComponent(int index) {
	 ...
	 *             point is not greater than its minimum point.
	 */
	public void addComponent(TriangleNet3DComponent comp) {
		// set the element ids for this net
		Iterator it = comp.getElementsViaRecursion().iterator();
		while (it.hasNext())
			((TriangleNet3DElement) it.next()).setID(this.nextElementID());

		// set component id
		int id = this.nextComponentID();
		comp.setComponentID(id);

		TriangleNet3DComponent[] comps = getComponents();

		TriangleNet3DComponent[] temp;
		if (comps == null) {
			temp = new TriangleNet3DComponent[1];
			temp[0] = comp;
		} else {
			temp = new TriangleNet3DComponent[comps.length + 1];
			for (int i = 0; i < comps.length; i++)
				temp[i] = comps[i];
	 ...
	 *             point is not greater than its minimum point.
	 */
	public void removeComponent(TriangleNet3DComponent comp) {
		TriangleNet3DComponent[] comps = getComponents();
		TriangleNet3DComponent[] temp = new TriangleNet3DComponent[comps.length - 1];
		int x = 0;
		for (int i = 0; i < comps.length; i++) {
			if (comps[i] != comp) {
				temp[x] = comps[i];
	 ...
	 *             point is not greater than its minimum point.
	 */
	public TriangleNet3DComponent createComponent() {
		TriangleNet3DComponent comp = new TriangleNet3DComponent(getScalarOperator()
				.copy());
		addComponent(comp);
		// Here an IllegalArgumentException can be thrown.
		return comp;
	...
	public TriangleNet3D splitTriangleNet(int[] indexes, ScalarOperator sop) { // Dag
		TriangleNet3DComponent[] newTriangleNetComps = new TriangleNet3DComponent[indexes.length];
	...
	public int getEuler() { // Dag
		...
		TriangleNet3DComponent[] comps = this.getComponents();
		int euler = 0;
		for (int i = 0; i < comps.length; i++)
	 ...
	 */
	public byte getType() {
		return ComplexGeoObject.TRIANGLE_NET_3D;
	}
	 ...
	 *             point is not greater than its minimum point.
	 */
	protected void setComponents(TriangleNet3DComponent[] comps) {
	...
	public SimpleGeoObj getElement(int id) {
		for (TriangleNet3DComponent comp : components) {
