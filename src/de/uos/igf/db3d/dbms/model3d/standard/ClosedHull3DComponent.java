...
import de.uos.igf.db3d.dbms.model3d.api.Volume3D;
 ...
 */
public class ClosedHull3DComponent implements ComplexGeoObject,
		Serializable {
	/* the wrapped triangle net component */
	private TriangleNet3DComponent tNetComp;
	 ...
	 */
	protected ClosedHull3DComponent(ClosedHull3D backRef, TriangleNet3DComponent tNetComp) {
	 ...
	 *             point is not greater than its minimum point.
	 */
	public TriangleNet3DElement removeElt(Triangle3D elt) throws UpdateException {
	...
	private boolean isInside(Point3D point) {
		...
		TriangleNet3DElement closest = null;
		Point3D pt = null;
		while (it.hasNext()) {
			TriangleNet3DElement tri = (TriangleNet3DElement) it.next();
	 ...
	 * Vector3D.
	 */
	private boolean isInside(Point3D point, TriangleNet3DElement tri) {
	 ...
	 * Vector3D.
	 */
	private boolean isInside(Point3D from, Point3D to, TriangleNet3DElement tri) {
	 ...
	 */
	public byte getType() {
		return ComplexGeoObject.COMP_CLOSED_HULL_3D;
	}
	...
	private double calculateSignedVolume() {
		...
		double volume = 0;
		Vector3D[] vec = null;
		TriangleNet3DElement elt = null;

		Vector3D center = this.getMBB().getCenter().getVector();

		while (it.hasNext()) {
			elt = (TriangleNet3DElement) it.next();
			vec = elt.getVectors();
	...
	protected TriangleNet3DComponent getTNetComp() {
		return tNetComp;
	}
