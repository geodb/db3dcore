...
import de.uos.igf.db3d.dbms.model3d.api.Solid3D;
...
public class TetrahedronNet3D extends SpatialObject3D implements Solid3D,
	...
	/** components of this */
	protected TetrahedronNet3DComponent[] components;
	 ...
	 */
	protected TetrahedronNet3D(TetrahedronNet3DComponent[] components,
			ScalarOperator sop) {
		super();
	...
	public TetrahedronNet3D(TetrahedronNet3D net) {
		super();
		TetrahedronNet3DComponent[] comps = net.getComponents();
		this.components = new TetrahedronNet3DComponent[comps.length];
	 ...
	 */
	public TetrahedronNet3DComponent[] getComponents() {
		// ObjectStore.fetch(this);
		// ObjectStore.fetch(components);
		if (components != null)
			return components;
		else
			return new TetrahedronNet3DComponent[0];
	}
	 ...
	 */
	public TetrahedronNet3DComponent getComponent(int index) {
	 ...
	 *             point is not greater than its minimum point.
	 */
	public void addComponent(TetrahedronNet3DComponent comp) {
		// set the element ids for this net
		Iterator it = comp.getElementsViaRecursion().iterator();
		while (it.hasNext())
			((TetrahedronNet3DElement) it.next()).setID(this.nextElementID());

		// set component id
		comp.setComponentID(this.nextComponentID());

		TetrahedronNet3DComponent[] comps = getComponents();
		TetrahedronNet3DComponent[] temp = new TetrahedronNet3DComponent[comps.length + 1];
		for (int i = 0; i < comps.length; i++)
			temp[i] = comps[i];
	 ...
	 *             point is not greater than its minimum point.
	 */
	public void removeComponent(TetrahedronNet3DComponent comp) {
		TetrahedronNet3DComponent[] comps = getComponents();
		TetrahedronNet3DComponent[] temp = new TetrahedronNet3DComponent[comps.length - 1];
		int x = 0;
		for (int i = 0; i < comps.length; i++) {
	 ...
	 *             point is not greater than its minimum point.
	 */
	public TetrahedronNet3DComponent createComponent() {
		TetrahedronNet3DComponent comp = new TetrahedronNet3DComponent(
	...
	public TetrahedronNet3D splitTetrahedronNet(int[] indexes,
		...
		TetrahedronNet3DComponent[] newTetrahedronNetComps = new TetrahedronNet3DComponent[indexes.length];
	...
	public int getEuler() { // Dag
		...
		TetrahedronNet3DComponent[] comps = this.getComponents();
		int euler = 0;
		for (int i = 0; i < comps.length; i++)
	 ...
	 */
	public byte getType() {
		return ComplexGeoObject.TETRAHEDRON_NET_3D;
	}
	 ...
	 *             point is not greater than its minimum point.
	 */
	protected void setComponents(TetrahedronNet3DComponent[] comps) {
	...
	public SimpleGeoObj getElement(int id) {
		for (TetrahedronNet3DComponent comp : components) {
