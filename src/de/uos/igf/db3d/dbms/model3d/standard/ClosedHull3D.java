...
import de.uos.igf.db3d.dbms.model3d.api.Hull3D;
...
public class ClosedHull3D extends SpatialObject3D implements Hull3D,
	...
	private ClosedHull3DComponent[] components;
	 ...
	 */
	public ClosedHull3DComponent getComponent(int index) {
	 ...
	 * @return ClosedHull3DComp[]
	 */
	public ClosedHull3DComponent[] getComponents() {
	 ...
	 */
	public byte getType() {
		return ComplexGeoObject.CLOSED_HULL_3D;
	}
	 ...
	 *             point is not greater than its minimum point.
	 */
	public void addComponent(ClosedHull3DComponent comp) {
		getTNet().addComponent(comp.getTNetComp());
		// Here an IllegalArgumentException can be thrown.

		// add also to the wrapped array
		ClosedHull3DComponent[] comps = getComponents();
		ClosedHull3DComponent[] temp = new ClosedHull3DComponent[comps.length + 1];
	 ...
	 *             point is not greater than its minimum point.
	 */
	public void removeComponent(ClosedHull3DComponent comp) {
	...
	private void buildComponents() {
		this.components = new ClosedHull3DComponent[this.countComponents()];
		for (int i = 0; i < this.components.length; i++)
			this.components[i] = new ClosedHull3DComponent(this, tnet
					.getComponent(i));
	}
	...
	public SimpleGeoObj getElement(int id) {
		for (ClosedHull3DComponent comp : components) {
			/*
