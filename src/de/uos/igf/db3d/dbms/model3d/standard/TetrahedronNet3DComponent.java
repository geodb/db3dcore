 ...
 */
public class TetrahedronNet3DComponent implements ComplexGeoObject,
		Serializable {
	...
	static {
		serialPersistentFields = new ObjectStreamField[] {
				...
				new ObjectStreamField("entry", TetrahedronNet3DElement.class),
	...
	/* entry element */
	private TetrahedronNet3DElement entry;
	 ...
	 *            ScalarOperator
	 */
	protected TetrahedronNet3DComponent(ScalarOperator sop) {
		this.id = -1;
		this.sop = sop;
	 ...
	 */
	protected TetrahedronNet3DComponent(ScalarOperator sop,
			TetrahedronNet3DElement[] elements) throws DB3DException {
		this.id = -1;
		this.sop = sop;
		this.sam = new RStar(MAX_SAM, sop);
		loadSAM(elements);
	 ...
	 *             class Vector3D.
	 */
	public TetrahedronNet3DElement addElt(Tetrahedron3D elt) throws DB3DException {

		TetrahedronNet3DElement element = new TetrahedronNet3DElement(elt);
		...
		// simultaneously
		while (it.hasNext()) {
			TetrahedronNet3DElement tetraElt = (TetrahedronNet3DElement) it.next();
		...
		if (neighbourCounter > 0) {
			for (int i = 0; i < neighbourCounter; i++) {
				...
				((TetrahedronNet3DElement) hns[i].getObject(0)).setNeighbour(hns[i]
						.getIndex(0), ((TetrahedronNet3DElement) hns[i].getObject(1)));
				((TetrahedronNet3DElement) hns[i].getObject(1)).setNeighbour(hns[i]
						.getIndex(1), ((TetrahedronNet3DElement) hns[i].getObject(0)));
			}
			// add element to SAM
			this.getSAM().insert(element);
			return element;
	 ...
	 *             point is not greater than its minimum point.
	 */
	public TetrahedronNet3DElement removeElt(Tetrahedron3D elt) throws DB3DException { // Dag

		// look for geometrically equivalent element in this component
		TetrahedronNet3DElement removable = null;
		Set set = this.getSAM().intersects(elt.getMBB());
		Iterator it = set.iterator();
		while (it.hasNext()) {
			TetrahedronNet3DElement current = (TetrahedronNet3DElement) it.next();
		...
		if (removable != null) {
			...
			if (!removable.hasNeighbours()) {
				...
				TetrahedronNet3DElement[] neighbour = removable.getNeighbours();
	 ...
	 */
	public boolean isConnectedWith(TetrahedronNet3DElement start, TetrahedronNet3DElement end) {
	 ...
	 * @return boolean - whether elements are connected or not.
	 */
	private boolean isConnectedWith(TetrahedronNet3DElement start,
			TetrahedronNet3DElement end, FlagMap flags) {

		flags.add(start); // set visited
		for (int i = 0; i < 4; i++) {
			TetrahedronNet3DElement nb = start.getNeighbour(i);
			if (nb != null) {
	...
	public boolean containsElt(Tetrahedron3D tet) { // Dag
		...
		Iterator it = tetras.iterator();
		while (it.hasNext()) {
			TetrahedronNet3DElement tetra = (TetrahedronNet3DElement) it.next();
	 ...
	 */
	public TetrahedronNet3DElement getEntryElement() {
		return this.entry;
	}
	...
	private Set getTriangles(Set tetraSet) { // Dag
		TetrahedronNet3DElement[] tetras = (TetrahedronNet3DElement[]) tetraSet
				.toArray(new TetrahedronNet3DElement[tetraSet.size()]);
	...
	private Set getPoints(Set tetras) { // Dag
		...
		Iterator it = tetras.iterator();
		TetrahedronNet3DElement tetra;
		while (it.hasNext()) {
			tetra = (TetrahedronNet3DElement) it.next();
			for (int j = 0; j < 4; j++)
	...
	public boolean isBorderVertex(Point3D point) { // Dag
		...
		Iterator it = tetras.iterator();
		while (it.hasNext()) {
			TetrahedronNet3DElement tet = (TetrahedronNet3DElement) it.next();
	 ...
	 * Vector3D.
	 */
	private TetrahedronNet3DElement getTetraContainingPoint(Point3D point) { // Dag
	...
	public boolean containsInside(Segment3D seg) {
		...
		TetrahedronNet3DElement current = getTetraContainingPoint(seg.getPoint(0));
	 ...
	 */
	public byte getType() {
		return ComplexGeoObject.COMP_TETRAHEDRON_NET_3D;
	}
	 ...
	 */
	public TetrahedronNet3DComponent serializationCopy() {
		try {
			...
			os.reset();
			os.close();
			return (TetrahedronNet3DComponent) ret;
		} catch (IOException e) {
			e.printStackTrace();
			return null;
	 ...
	 * MBB3D whose maximum point is not greater than its minimum point.
	 */
	private void loadSAM(TetrahedronNet3DElement[] elements) {
	 ...
	 *             getPoint(int) of the class Tetrahedron3D.
	 */
	public void buildNetTopology(TetrahedronNet3DElement[] elts) throws DB3DException {
		...
		for (int i = 0; i < elts.length; i++) {
			if (elts[i].isInterior() != true) {
				...
				Point3D po = null;
				Iterator it;
				TetrahedronNet3DElement te = null;
				...
				for (int m = 0; m < 4; m++) {
					...
					if ((elts[i].getNeighbour(m)) == null) {
						...
						while (it.hasNext()) {
							TetrahedronNet3DElement oTE = (TetrahedronNet3DElement) it.next();
	...
	protected void updateEntryElement() {
		...
		Iterator it = set.iterator();
		while (it.hasNext()) {
			TetrahedronNet3DElement tetelt = (TetrahedronNet3DElement) it.next();
	 ...
	 */
	private void setEntryElement(TetrahedronNet3DElement tet) {
		this.entry = tet;
	}
	...
	protected void updateEulerStatistics() {
		...
		Triangle3D tri = null;
		TetrahedronNet3DElement tetelt = null;
		Iterator it = set.iterator();

		while (it.hasNext()) {
			tetelt = (TetrahedronNet3DElement) it.next();
			for (int j = 0; j < 4; j++) {
	...
	public Set<Triangle3D> findTinBorder1() {
		...
		TetrahedronNet3DElement tetr = null;
		...
		while (it.hasNext()) {
			...
			tetr = (TetrahedronNet3DElement) it.next();
	...
	private void tinBorderViaRecursion(Set<Triangle3D> outerTriangles,
			Set<Tetrahedron3D> visited, TetrahedronNet3DElement tetr) {
		...
		for (int i = 0; i < 4; i++) {
			if (tetr.getNeighbour(i) == null) {
				...
				TetrahedronNet3DElement nb = tetr.getNeighbour(i);
	 ...
	 * Vector3D.
	 */
	private void getOuterNeighbourViaRecursion(TetrahedronNet3DElement nb,
			Segment3D seg) {
		for (int i = 0; i < 4; i++) {
			TetrahedronNet3DElement nb2 = nb.getNeighbour(i);
	 ...
	 * Vector3D.
	 */
	private Segment3D getCommonSegment(Triangle3D tri, TetrahedronNet3DElement nb) {
	 ...
	 * Vector3D.
	 */
	private boolean haveCommonSegment(Segment3D seg, TetrahedronNet3DElement nb) {
		Triangle3D[] nbTri = nb.getTriangles();
		Segment3D[] nbSeg = new Segment3D[9];
		// It is enough to regard just 3 triangles of the tetrahedron,
		// because the segments repeat.
	 ...
	 */
	private void makeSet(Set set, TetrahedronNet3DElement elt) {

		// make visited
		set.add(elt);

		for (int i = 0; i < 4; i++) {
			TetrahedronNet3DElement nb = elt.getNeighbour(i);
	...
	public static class TetrahedronElt3DIterator {
		...
		/* the next element */
		private TetrahedronNet3DElement next;
		 ...
		 */
		private TetrahedronElt3DIterator(TetrahedronNet3DElement element) {
		...
		public boolean hasNext() {
			TetrahedronNet3DElement tri = recursiveFunction();
			if (tri != null) {
				if (start)
					start = false;
				next = tri;
		 ...
		 */
		public TetrahedronNet3DElement next() {
			return next;
		}
		 ...
		 */
		private TetrahedronNet3DElement recursiveFunction() {
			...
			while (!stack.isEmpty() || start) {
				...
				if (actual.i < 3) {
					do {
						actual.incI();
						TetrahedronNet3DElement tri = actual.element
		...
		private static class Actual {
			...
			private TetrahedronNet3DElement element;
			 ...
			 */
			private Actual(TetrahedronNet3DElement element) {
	...
	public SimpleGeoObj getElement(int id) {
		...
		Iterator<TetrahedronNet3DElement> it = getElementsViaRecursion().iterator();
		while (it.hasNext()) {
			TetrahedronNet3DElement tet = it.next();
			if (tet.getID() == id)
				return tet;
		}
		return null;
