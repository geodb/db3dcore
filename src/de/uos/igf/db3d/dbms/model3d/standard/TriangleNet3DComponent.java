...
import de.uos.igf.db3d.dbms.model3d.api.ComplexGeoObject;
 ...
 */
public class TriangleNet3DComponent implements ComplexGeoObject,
		Serializable {
	...
	static {
		serialPersistentFields = new ObjectStreamField[] {
				...
				new ObjectStreamField("entry", TriangleNet3DElement.class),
	...
	/* entry element */
	private TriangleNet3DElement entry;
	 ...
	 */
	protected TriangleNet3DComponent(ScalarOperator sop) {
		this.id = -1;
		this.sop = sop;
		this.oriented = false;
	 ...
	 *             Triangle3D.
	 */
	public TriangleNet3DComponent(ScalarOperator sop, TriangleNet3DElement[] elements) {
		this.id = -1;
		this.sop = sop;
		this.sam = new RStar(MAX_SAM, sop);
	 ...
	 *             class Vector3D.
	 */
	public TriangleNet3DElement addElt(Triangle3D elt) throws UpdateException {
		TriangleNet3DElement element = new TriangleNet3DElement(elt);
		...
		Set<Equivalentable> triangles = (Set<Equivalentable>) this.getSAM().intersects(elt.getMBB());
		...
		while (it.hasNext()) { // test for intersections
			TriangleNet3DElement triElt = (TriangleNet3DElement) it.next();
		...
		if (neighbourCounter > 0) {
			for (int i = 0; i < neighbourCounter; i++) {
				...
				((TriangleNet3DElement) hns[i].getObject(0)).setNeighbour(
						hns[i].getIndex(0),
						((TriangleNet3DElement) hns[i].getObject(1)));
				((TriangleNet3DElement) hns[i].getObject(1)).setNeighbour(
						hns[i].getIndex(1),
						((TriangleNet3DElement) hns[i].getObject(0)));
			}
			// add element to SAM
			this.getSAM().insert(element);
			return element;
	 ...
	 *             point is not greater than its minimum point.
	 */
	public TriangleNet3DElement removeElt(Triangle3D elt) throws UpdateException { // Dag

		// find element and set removable
		TriangleNet3DElement removable = null;
		Set<Equivalentable> set = (Set<Equivalentable>) this.getSAM().intersects(elt.getMBB());
		Iterator<Equivalentable> it = set.iterator();
		while (it.hasNext()) {
			TriangleNet3DElement current = (TriangleNet3DElement) it.next();
		...
		if (removable != null) {
			if (!removable.hasNeighbours()) {
				...
				TriangleNet3DElement[] neighbour = removable.getNeighbours();
	 ...
	 *             point is not greater than its minimum point.
	 */
	public TriangleNet3DElement removeEltWithoutTopologyCheck(Triangle3D elt)
			throws UpdateException {
	 ...
	 *             class Vector3D.
	 */
	public TriangleNet3DElement addEltWithoutTopologyCheck(Triangle3D elt)
			throws UpdateException {
		TriangleNet3DElement element = new TriangleNet3DElement(elt);
		...
		for (int i = 0; i < neighbourCounter; i++) {
			// set neighborly relations
			((TriangleNet3DElement) hns[i].getObject(0)).setNeighbour(
					hns[i].getIndex(0), ((TriangleNet3DElement) hns[i].getObject(1)));
			((TriangleNet3DElement) hns[i].getObject(1)).setNeighbour(
					hns[i].getIndex(1), ((TriangleNet3DElement) hns[i].getObject(0)));
		}
		// add element to SAM
		this.getSAM().insert(element);
		return element;
	 ...
	 */
	public boolean isConnectedWith(TriangleNet3DElement start, TriangleNet3DElement end) {
	 ...
	 * @return boolean - whether elements are connected or not.
	 */
	private boolean isConnectedWith(TriangleNet3DElement start, TriangleNet3DElement end,
			FlagMap flags) {
		flags.add(start);

		for (int i = 0; i < 3; i++) {
			TriangleNet3DElement nb = start.getNeighbour(i);
			if (nb != null) {
	...
	public boolean containsElt(Triangle3D tri) { // Dag
		Set<Equivalentable> triangles = (Set<Equivalentable>) this.getSAM().intersects(tri.getMBB());
	...
	public boolean containsElt(Segment3D seg) { // Dag
		Set<Equivalentable> set = (Set<Equivalentable>) this.getSAM().intersects(seg.getMBB());
	...
	public boolean containsElt(Point3D point) { // Dag
		Set<Equivalentable> set = (Set<Equivalentable>) this.getSAM().intersects(point.getMBB());
	...
	public int countBorderVertices() { // Dag
		...
		Iterator<Equivalentable> it = set.iterator();
		while (it.hasNext()) {
			TriangleNet3DElement triangle = (TriangleNet3DElement) it.next();
			for (int j = 0; j < 3; j++)
	 ...
	 */
	public TriangleNet3DElement getEntryElement() {
		return this.entry;
	}
	...
	private Set<Equivalentable> getSegments(Set<Equivalentable> triangleSet) { // Dag
		...
		TriangleNet3DElement[] triangles = triangleSet
				.toArray(new TriangleNet3DElement[triangleSet.size()]);
	 ...
	 */
	public TriangleNet3DComponent serializationCopy() {
		try {
			...
			os.reset();
			os.close();
			return (TriangleNet3DComponent) ret;
		} catch (IOException e) {
			e.printStackTrace();
			return null;
	...
	public boolean intersects(Plane3D plane) {// Dag
		...
		if (sgo == null)
			return false;
		Set<Equivalentable> set = (Set<Equivalentable>) this.getSAM().intersects(sgo.getMBB());
		Iterator<Equivalentable> it = set.iterator();
		while (it.hasNext()) {
			TriangleNet3DElement tri = (TriangleNet3DElement) it.next();
	...
	public boolean intersects(MBB3D mbb) { // Dag
		...
		Set<Equivalentable> set = (Set<Equivalentable>) this.getSAM().intersects(mbb);
		Iterator<Equivalentable> it = set.iterator();
		while (it.hasNext()) {
			TriangleNet3DElement triElt = (TriangleNet3DElement) it.next();
	...
	public boolean contains(Segment3D seg) { // Dag
		...
		Set<Equivalentable> triangles = (Set<Equivalentable>) this.getSAM().intersects(mbb);
	...
	public void makeOrientationConsistent(ScalarOperator sop) { // Dag
		...
		TriangleNet3DElement triangle = getEntryElement();
	...
	public boolean isBorderFace(Triangle3D triangle) { // Dag
		...
		TriangleNet3DElement reference = null;
		Set<Equivalentable> set = (Set<Equivalentable>) this.getSAM().intersects(triangle.getMBB());
		Iterator<Equivalentable> it = set.iterator();
		while (it.hasNext()) {
			TriangleNet3DElement current = (TriangleNet3DElement) it.next();
			if (current
	 ...
	 * 
	 * @return byte - type.
	 * @see db3d.dbms.model3d.ComplexGeoObject#getType()
	 */
	public byte getType() {
		return ComplexGeoObject.COMP_TRIANGLE_NET_3D;
	}
	 ...
	 */
	private void loadSAM(TriangleNet3DElement[] elements) {
	 ...
	 *             Triangle3D.
	 */
	public void buildNetTopology(TriangleNet3DElement[] elts) {
		ScalarOperator so = getScalarOperator();

		for (int i = 0; i < elts.length; i++) {
			if (elts[i].isInterior() != true) {
				SAM sa = this.getSAM();
				Set<Equivalentable> query = (Set<Equivalentable>) sa.intersects(elts[i].getMBB());
				query.remove(elts[i]);
				...
				for (int m = 0; m < 3; m++) {
					...
					if ((elts[i].getNeighbour(m)) == null) {
						...
						while (it.hasNext()) {
							TriangleNet3DElement oTE = (TriangleNet3DElement) it.next();
	 ...
	 *             Triangle3D.
	 */
	public void checkNetTopology(TriangleNet3DElement[] elts) {
		...
		for (int i = 0; i < elts.length; i++) {
			if (this.isEltInterior(elts[i], flags) != true) {
				Set<Equivalentable> query = (Set<Equivalentable>) sam.intersects(elts[i].getMBB());
				query.remove(elts[i]);
				if (flags.checkFlag(elts[i], FlagMap.F1) == true) { // neighbour
					// 0
					TriangleNet3DElement el = elts[i].getNeighbour(0);
					query.remove(el);
				}
				if (flags.checkFlag(elts[i], FlagMap.F2) == true) { // neighbour
					// 1
					TriangleNet3DElement el = elts[i].getNeighbour(1);
					query.remove(el);
				}
				if (flags.checkFlag(elts[i], FlagMap.F3) == true) { // neighbour
					// 2
					TriangleNet3DElement el = elts[i].getNeighbour(2);
					query.remove(el);
				}
				for (int j = 0; j < 3; j++) { // for every side of the triangle
					...
					if (flags.checkFlag(elts[i], sh) != true) {
						...
						if (elts[i].hasNeighbour(j)) {
							TriangleNet3DElement el = elts[i].getNeighbour(j);
	 ...
	 * @param flags FlagMap
	 */
	private boolean isEltInterior(TriangleNet3DElement elt, FlagMap flags) {
	...
	protected void updateEntryElement() {
		...
		if (closed) // if the net is closed take one arbitrary triangle
			setEntryElement((TriangleNet3DElement) set.iterator().next());
	}
	 ...
	 */
	private void setEntryElement(TriangleNet3DElement tri) {
		this.entry = tri;
	}
	...
	protected void updateEulerStatistics() {
		...
		Iterator<?> it = set.iterator();
		TriangleNet3DElement trielt = null;
		int cnt = 0;
		double time = System.currentTimeMillis();
		int size = set.size();
		while (it.hasNext()) {
			...
			trielt = (TriangleNet3DElement) it.next();
			for (int j = 0; j < 3; j++) {
	 ...
	 */
	private void makeSet(Set<Equivalentable> set, TriangleNet3DElement elt) {

		LinkedList<TriangleNet3DElement> toVisit = new LinkedList<TriangleNet3DElement>();
		TreeSet<Integer> checked = new TreeSet<Integer>();
		toVisit.add(elt);
		checked.add(elt.getID());

		TriangleNet3DElement currTri;
		TriangleNet3DElement nb;
	...
	public Set<Segment3D> findTinBorder1() {
		...
		while (it.hasNext()) {
			...
			tri = (TriangleNet3DElement) it.next();
	...
	public Set<Segment3D> findTinBorder2() {
		...
		TriangleNet3DElement first = tri; // first points to the first outer triangle
	...
	public Collection<Segment3D> getAllSegmentsWithPoint(Point3D point) {
		...
		Collection<TriangleNet3DElement> allTrianglesWithPoint = getAllTrianglesWithPoint2(point);
		Segment3D seg;
		for (TriangleNet3DElement tri : allTrianglesWithPoint) {
			for (int i = 0; i < 3; i++) {
	 ...
	 *             class Vector3D.
	 */
	public Set<TriangleNet3DElement> getAllTrianglesWithPoint(Point3D point) {
		Set<TriangleNet3DElement> resultSet = new HashSet<TriangleNet3DElement>();

		TriangleNet3DElement first = (TriangleNet3DElement) this.getSAM().nearest(1, point)[0]
				.getObjectRef();
		TriangleNet3DElement current = null;
		TriangleNet3DElement prev = first;
		...
		if (current == null) {
			resultSet = new HashSet<TriangleNet3DElement>();
			boolean goon = true;
			...
			while (goon) {
				...
				TriangleNet3DElement[] nearestTris = new TriangleNet3DElement[nearestResult.length];
				for (int i = 0; i < nearestResult.length; i++) {
					nearestTris[i] = (TriangleNet3DElement) nearestResult[i]
							.getObjectRef();
				}
	...
	public Collection<TriangleNet3DElement> getAllTrianglesWithPoint2(Point3D point) {

		LinkedList<TriangleNet3DElement> result = new LinkedList<TriangleNet3DElement>();

		TriangleElt3DIterator triEltsIt = this.getElementsIterator();

		TriangleNet3DElement nextTri;
	...
	public Map<Point3D, List<Point3D>> createPointIndex() {
		...
		for (Object el : elements) {
			TriangleNet3DElement tri = (TriangleNet3DElement) el;
			count++;
	...
	public static final class TriangleElt3DIterator {
		...
		/* the next element */
		private TriangleNet3DElement next;
		 ...
		 * element
		 */
		private TriangleElt3DIterator(TriangleNet3DElement element) {
		...
		public boolean hasNext() {
			TriangleNet3DElement tri = recursiveFunction();
			if (tri != null) {
				if (start)
					start = false;
				next = tri;
		 ...
		 */
		public TriangleNet3DElement next() {
			return next;
		}
		 ...
		 */
		private TriangleNet3DElement recursiveFunction() {
			...
			while (!stack.isEmpty() || start) {
				...
				if (actual.i < 2) {
					do {
						actual.incI();
						TriangleNet3DElement tri = actual.element
		...
		public static final class Actual {
			...
			private TriangleNet3DElement element;
			 ...
			 */
			private Actual(TriangleNet3DElement element) {
	 ...
	 */
	public TriangleNet3DElement getElement(int id) {

		Iterator<TriangleNet3DElement> it = getElementsViaRecursion().iterator();
		while (it.hasNext()) {
			TriangleNet3DElement t = it.next();
			if (t.getID() == id)
				return t;
		}
		return null;
