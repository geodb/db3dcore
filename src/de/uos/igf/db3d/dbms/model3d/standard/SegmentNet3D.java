...
import de.uos.igf.db3d.dbms.model3d.api.Curve3D;
...
public class SegmentNet3D extends SpatialObject3D implements Curve3D,
	...
	/* components of this */
	private SegmentNet3DComponent[] components;
	 ...
	 */
	protected SegmentNet3D(SegmentNet3DComponent[] components, ScalarOperator sop) {
		super();
	...
	public SegmentNet3D(SegmentNet3D net) {
		super();
		SegmentNet3DComponent[] comps = net.getComponents();
		this.components = new SegmentNet3DComponent[comps.length];
	 ...
	 */
	public SegmentNet3DComponent[] getComponents() {
		// ObjectStore.fetch(this);
		// ObjectStore.fetch(components);
		if (components != null)
			return components;
		else {
			components = new SegmentNet3DComponent[0];
			return components;
		}
	}
	 ...
	 *         index position.
	 */
	public SegmentNet3DComponent getComponent(int index) {
		if (components == null)
			components = new SegmentNet3DComponent[0];
	 ...
	 *             point is not greater than its minimum point.
	 */
	public void addComponent(SegmentNet3DComponent comp) {
		// set the element ids for this net
		Iterator it = comp.getElementsViaRecursion().iterator();
		while (it.hasNext())
			((SegmentNet3DElement) it.next()).setID(this.nextElementID());

		// set component id
		comp.setComponentID(this.nextComponentID());

		SegmentNet3DComponent[] comps = getComponents();
		SegmentNet3DComponent[] temp = new SegmentNet3DComponent[comps.length + 1];
		for (int i = 0; i < comps.length; i++)
			temp[i] = comps[i];
	 ...
	 *             point is not greater than its minimum point.
	 */
	public void removeComponent(SegmentNet3DComponent comp) {
		SegmentNet3DComponent[] comps = getComponents();
		SegmentNet3DComponent[] temp = new SegmentNet3DComponent[comps.length - 1];
		int x = 0;
		for (int i = 0; i < comps.length; i++) {
	 ...
	 *             point is not greater than its minimum point.
	 */
	public SegmentNet3DComponent createComponent() {
		SegmentNet3DComponent comp = new SegmentNet3DComponent(getScalarOperator().copy());
		addComponent(comp);
		return comp;
	}
	...
	public SegmentNet3D splitSegmentNet(int[] indexes, ScalarOperator sop) {// Dag
		SegmentNet3DComponent[] newSegmentNetComps = new SegmentNet3DComponent[indexes.length];
	...
	public int getEuler() {
		...
		SegmentNet3DComponent[] comps = this.getComponents();
		int euler = 0;
	 ...
	 */
	public byte getType() {
		return ComplexGeoObject.SEGMENT_NET_3D;
	}
	 ...
	 *             point is not greater than its minimum point.
	 */
	protected void setComponents(SegmentNet3DComponent[] comps) {
	...
	public SimpleGeoObj getElement(int id) {
		for (SegmentNet3DComponent comp : components) {
