 ...
 */
public class PointNet3DComponent implements ComplexGeoObject,
		Serializable {
	 ...
	 *            ScalarOperator
	 */
	public PointNet3DComponent(ScalarOperator sop) {
		this.id = -1;
		this.sop = sop;
	 ...
	 */
	protected PointNet3DComponent(PointNet3DElement[] elements, ScalarOperator sop) {
		this.id = -1;
		this.sop = sop;
		this.sam = new RStar(MAX_SAM, sop);
		loadSAM(elements);
	...
	public boolean intersects(Plane3D plane) { // Dag
		...
		PointNet3DElement[] points = (PointNet3DElement[]) set.toArray(new PointNet3DElement[set
				.size()]);
	 ...
	 *             point is not greater than its minimum point.
	 */
	public PointNet3DElement addElt(Point3D point) throws UpdateException {
		PointNet3DElement elt = new PointNet3DElement(point);
	 ...
	 *             point is not greater than its minimum point.
	 */
	public PointNet3DElement removeElt(Point3D elt) throws UpdateException {
		// find element
		PointNet3DElement removable = null;
		Set set = this.getSAM().intersects(elt.getMBB());
		Iterator it = set.iterator();
		while (it.hasNext()) {
			PointNet3DElement current = (PointNet3DElement) it.next();
	...
	public boolean containsElt(Point3D point) {
		...
		Iterator it = set.iterator();
		while (it.hasNext()) {
			PointNet3DElement pointelt = (PointNet3DElement) it.next();
	 ...
	 */
	public PointNet3DComponent serializationCopy() {
		try {
			...
			os.reset();
			os.close();
			return (PointNet3DComponent) ret;
		} catch (IOException e) {
			e.printStackTrace();
			return null;
	 ...
	 */
	public byte getType() {
		return ComplexGeoObject.COMP_POINT_NET_3D;
	}
	 ...
	 */
	private void loadSAM(PointNet3DElement[] elements) {
	...
	public SimpleGeoObj getElement(int id) {
		...
		Iterator it = getElementsViaSAM().iterator();
		while (it.hasNext()) {
			PointNet3DElement p = (PointNet3DElement) it.next();
			if (p.getID() == id)
				return p;
		}
		return null;
