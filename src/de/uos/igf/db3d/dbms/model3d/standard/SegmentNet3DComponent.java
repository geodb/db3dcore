 ...
 */
public class SegmentNet3DComponent implements ComplexGeoObject,
		Serializable {
	...
	static {
		serialPersistentFields = new ObjectStreamField[] {
				...
				new ObjectStreamField("entry", SegmentNet3DElement.class),
	...
	private SegmentNet3DElement entry;
	 ...
	 *            ScalarOperator
	 */
	protected SegmentNet3DComponent(ScalarOperator sop) {
		this.id = -1;
		this.sop = sop;
		this.oriented = false;
	 ...
	 */
	public SegmentNet3DComponent(ScalarOperator sop, SegmentNet3DElement[] elements) {
		this.id = -1;
		this.sop = sop;
		this.sam = new RStar(MAX_SAM, sop);
		loadSAM(elements);
	...
	public boolean intersects(Plane3D plane) throws DB3DException { // Dag
		...
		while (it.hasNext()) {
			SegmentNet3DElement segment = (SegmentNet3DElement) it.next();
	...
	public boolean intersects(MBB3D mbb) { // Dag
		...
		while (it.hasNext()) {
			SegmentNet3DElement segelt = (SegmentNet3DElement) it.next();
	 ...
	 */
	public SegmentNet3DElement getEndElement() {// Dag
		if (this.isClosed())
			return (this.getEntryElement().getNeighbour(1));
		SegmentNet3DElement current = this.getEntryElement();
	 ...
	 *             class Vector3D.
	 */
	public SegmentNet3DElement addElt(Segment3D elt) throws UpdateException { // Dag
	 ...
	 *             class Vector3D.
	 */
	public SegmentNet3DElement addElt(Point3D p1, Point3D p2) throws UpdateException { // Dag
		// if the netcomp is empty
		if (this.isEmpty()) {
			SegmentNet3DElement newElt = new SegmentNet3DElement(new Point3D(p1),
					new Point3D(p2), sop);
		...
		SegmentNet3DElement entry = this.getEntryElement();
		SegmentNet3DElement end = this.getEndElement();
		...
		SegmentNet3DElement newSeg = null;
		if (addAtEntry)
			newSeg = new SegmentNet3DElement(p_nonref, p_ref, sop);
		else
			newSeg = new SegmentNet3DElement(p_ref, p_nonref, sop);
	 ...
	 *             point is not greater than its minimum point.
	 */
	public SegmentNet3DElement removeElt(Segment3D elt) throws UpdateException { // Dag

		if (this.isClosed()) { // find element, set entry and remove it
			SegmentNet3DElement removable = null;
			Set set = this.getSAM().intersects(elt.getMBB());
			Iterator it = set.iterator();
			while (it.hasNext()) {
				SegmentNet3DElement current = (SegmentNet3DElement) it.next();
	...
	public double getLength() {
		...
		while (it.hasNext())
			length += ((SegmentNet3DElement) it.next()).getLength();
	 ...
	 */
	public SegmentNet3DElement getEntryElement() {
		return this.entry;
	}
	...
	public Set getElementsViaRecursion() {
		...
		if (this.isClosed()) { // if closed we must check for loop
			SegmentNet3DElement begin = current;
			do {
				set.add(current);
				current = current.getNeighbour(0);
			} while (current.hasNeighbour(0)
	...
	public Set getPoints() { // Dag
		...
		Iterator it = set.iterator();
		while (it.hasNext()) {
			SegmentNet3DElement seg = (SegmentNet3DElement) it.next();
			for (int j = 0; j < 2; j++)
	...
	public void makeOrientationConsistent() {
		...
		SegmentNet3DElement current;
		SegmentNet3DElement old = this.getEntryElement();
	 ...
	 */
	public byte getType() {
		return ComplexGeoObject.COMP_SEGMENT_NET_3D;
	}
	 ...
	 */
	public SegmentNet3DComponent serializationCopy() {
		try {
			...
			os.reset();
			os.close();
			return (SegmentNet3DComponent) ret;
		} catch (IOException e) {
			e.printStackTrace();
			return null;
	 ...
	 */
	private void loadSAM(SegmentNet3DElement[] elements) {
	 ...
	 *             getPoint(int) of the class Tetrahedron3D.
	 */
	public void buildNetTopology(SegmentNet3DElement[] elts) {
			...
			if (elts[i].isInterior() != true) {
				...
				for (int m = 0; m < 2; m++) {
					...
					if ((elts[i].getNeighbour(m)) == null) {
						...
						while (it.hasNext()) {
							SegmentNet3DElement oSE = (SegmentNet3DElement) it.next();
	...
	protected void updateEntryElement() {
		...
		if (this.getEntryElement() == null) { // every element was interior so
			...
			SAM.NNResult[] result = this.getSAM().nearest(1,
					this.getMBB().getPMin());
			setEntryElement((SegmentNet3DElement) result[0].getObjectRef());
		}
	}
	 ...
	 */
	private void setEntryElement(SegmentNet3DElement seg) {
		this.entry = seg;
	}
	...
	public static class SegmentElt3DIterator {
		private SegmentNet3DElement actual;
		private SegmentNet3DElement begin;

		private SegmentElt3DIterator(SegmentNet3DElement element) {
			actual = element;
			begin = element;
		}
		 ...
		 */
		public SegmentNet3DElement next() {
			SegmentNet3DElement temp = actual;
	...
	public SimpleGeoObj getElement(int id) {
		...
		Iterator<SegmentNet3DElement> it = getElementsViaRecursion().iterator();
		while (it.hasNext()) {
			SegmentNet3DElement seg = it.next();
			if (seg.getID() == id)
				return seg;
		}
		return null;
