...
import de.uos.igf.db3d.dbms.spatials.geometries3d.PointSet3D;
import de.uos.igf.db3d.dbms.spatials.geometries3d.Rectangle3D;
...
import de.uos.igf.db3d.dbms.spatials.geometries3d.TetrahedronSet3D;
...
public class Utils {
	 ...
	 * @return String -
	 */
	public static String getText(Geometry3D obj) {
		...
		case SEGMENT:
			...
			for (int i = 0; i < point.length; i++) {
				b.append("P:(" + point[i].getX() + "," + point[i].getY() + ","
						+ point[i].getZ() + ") ");
			}
			b.append("]");
			return b.toString();
		...
		case VECTOR:
			...
			text = "Vector3D[ vecP( " + vecPoint.getX() + "," + vecPoint.getY()
					+ "," + vecPoint.getZ() + " )";
		...
		case LINE:
			...
			text = "Line3D[ P( " + origin.getX() + "," + origin.getY() + ","
					+ origin.getZ() + " )";
			text = text + "VectorP( " + direcVecPoint.getX() + ","
					+ direcVecPoint.getY() + "," + direcVecPoint.getZ() + " )";
		...
		case TETRAHEDRON:
			...
			for (int i = 0; i < pts.length; i++) {
				b3.append("P" + i + ":(" + pts[i].getX() + "," + pts[i].getY()
						+ "," + pts[i].getZ() + ") ");
			}
			b3.append("]");
			return b3.toString();
		...
		case MBB:
			...
			text = "MBB3D[ Pmin(" + min.getX() + "," + min.getY() + ","
					+ min.getZ() + ") ";
			text = text + "Pmax(" + max.getX() + "," + max.getY() + ","
					+ max.getZ() + ") ]";
		...
		case PLANE:
			...
			text = "Plane3D[ NormalVector(" + normv.getX() + "," + normv.getY()
					+ "," + normv.getZ() + ") ";
			text = text + "PositionVector(" + posv.getX() + "," + posv.getY()
					+ "," + posv.getZ() + ") " + " ]";
			return text;
	...
	private static String getText(Triangle3D tri) {
		...
		b.append("Triangle3D[ ");
		for (int i = 0; i < point.length; i++) {
			b.append("P" + i + ":(" + point[i].getX() + "," + point[i].getY()
					+ "," + point[i].getZ() + ") ");
		}
		b.append("]");
	...
	public static String getText(Rectangle3D rec) {
		...
		b.append("Rec[ ");
		for (int i = 0; i < 3; i++)
			b.append("(" + rec.getPoint(i).getX() + ","
					+ rec.getPoint(i).getY() + "," + rec.getPoint(i).getZ()
					+ ") , ");

		b.append("(" + rec.getPoint(3).getX() + "," + rec.getPoint(3).getY()
				+ "," + rec.getPoint(3).getZ() + ") ]");
		return b.toString();
	}
	...
	private static String getText(Point3D p, int number) {
		String text = "P_" + number + ":(" + p.getX() + "," + p.getY() + ","
				+ p.getZ() + ") ";
		return text;
	}
	...
	private static String getText(Tetrahedron3D tet, int number) {
		...
		for (int i = 0; i < pts.length; i++) {
			b.append("P" + i + ":(" + pts[i].getX() + "," + pts[i].getY() + ","
					+ pts[i].getZ() + ") ");
		}
		b.append("]");
		return b.toString();
	...
	private static String writeSegment(Segment3D seg, double[] rgb) {
		...
		b.append(tab + "geometry IndexedLineSet { \n" + tab
				+ "coord Coordinate { \n" + tab + "point [ \n");
		// coordinates
		b.append(tab + "" + seg.getPoint(0).getCoord(0) + " "
				+ seg.getPoint(0).getCoord(1) + " "
				+ seg.getPoint(0).getCoord(2) + ",  #Index0\n" + tab + ""
				+ seg.getPoint(1).getCoord(0) + " "
				+ seg.getPoint(1).getCoord(1) + " "
				+ seg.getPoint(1).getCoord(2) + ",  #Index1\n");
	...
	private static String writeLine(Line3D line, double[] rgb) {
		...
		Point3D p1 = (Vector3D.add(projected.getVector(),
				(Vector3D.mult(line.getDVector(), 100)))).getAsPoint3D();
		Point3D p2 = (Vector3D.sub(projected.getVector(),
				(Vector3D.mult(line.getDVector(), 100)))).getAsPoint3D();

		Segment3D segment = new Segment3D(p1, p2, new GeoEpsilon(
	...
	private static String writeTriangleAsWireframe(Triangle3D tri, double[] rgb) {
		...
		for (int i = 0; i < 2; i++) {
			b.append(tab + "" + tri.getPoint(i).getCoord(0) + " "
					+ tri.getPoint(i).getCoord(1) + " "
					+ tri.getPoint(i).getCoord(2) + ",  #Index" + i + "\n");
		}
		b.append(tab + "" + tri.getPoint(2).getCoord(0) + " "
				+ tri.getPoint(2).getCoord(1) + " "
				+ tri.getPoint(2).getCoord(2) + "   #Index2 \n");

		b.append(tab + "] \n" + tab + "} \n" + tab + "coordIndex [ \n");

		b.append(tab + "0, 1, 2, 0 \n"); // coordIndexe werden statisch
											// geschrieben
	...
	private static String writeTriangleAsSolid(Triangle3D tri, double[] rgb) {
		...
		b.append("Transform {\n children [\n  Shape {\n  appearance Appearance {\n    material Material { emissiveColor 0 0 0  diffuseColor "
				+ rgb[0]
				+ " "
				+ rgb[1]
				+ " "
		...
		// for ( int i = 0; i < 2; i++) {
		// text +=
		// tab+""+tri.getPoint(i).getCoord(0)+" "+tri.getPoint(i).getCoord(1)+" "+tri.getPoint(i).getCoord(2)+",  #Index"+i+"\n";
		// }
		// text +=
		// tab+""+tri.getPoint(2).getCoord(0)+" "+tri.getPoint(2).getCoord(1)+" "+tri.getPoint(2).getCoord(2)+"   #Index2 \n";
		//
		// text+= tab+"] \n" +
		// tab+"} \n"+
		// tab+"coordIndex [ \n";
	 ...
	 * @return String - VRML-Group{} element
	 */
	private static String writeTetraAsWireframe(Tetrahedron3D tetra,
			double[] rgb) {
		StringBuffer b = new StringBuffer();
		...
		for (int i = 0; i < 3; i++) {
			b.append(tab + "" + tetra.getPoint(i).getCoord(0) + " "
					+ tetra.getPoint(i).getCoord(1) + " "
					+ tetra.getPoint(i).getCoord(2) + ",  #Index" + i + "\n");
		}
		b.append(tab + "" + tetra.getPoint(3).getCoord(0) + " "
				+ tetra.getPoint(3).getCoord(1) + " "
				+ tetra.getPoint(3).getCoord(2) + "   #Index3 \n");
	...
	private static String writeTetraAsSolid(Tetrahedron3D tetra, double[] rgb) {
		...
		b.append("Transform {\n children [\n  Shape {\n  appearance Appearance {\n    material Material { emissiveColor 0.5 0.5 0  diffuseColor "
				+ rgb[0]
				+ " "
				+ rgb[1]
				+ " "
	...
	/**
	 * Erzeugt VRMLcode fuer ein Wireframe3D Objekt (nur in 'WIREFRAME'
	 * Darstellung moeglich)
	 * 
	 * @return String - VRML-Group{} element
	 */
	private static String writeWireframe(WireframeGeometry3D wf, double[] rgb) {
		StringBuffer b = new StringBuffer();
		...
		for (int i = 0; i < count; i++) {
			b.append(tab + "" + seg[i].getPoint(0).getCoord(0) + " "
					+ seg[i].getPoint(0).getCoord(1) + " "
					+ seg[i].getPoint(0).getCoord(2) + ",  #Index" + counter
					+ "\n");
			counter++;
			b.append(tab + "" + seg[i].getPoint(1).getCoord(0) + " "
					+ seg[i].getPoint(1).getCoord(1) + " "
					+ seg[i].getPoint(1).getCoord(2) + ",  #Index" + counter
					+ "\n");
			counter++;
		}
		b.append(tab + "" + seg[count].getPoint(0).getCoord(0) + " "
				+ seg[count].getPoint(0).getCoord(1) + " "
				+ seg[count].getPoint(0).getCoord(2) + "  #Index" + counter
				+ "\n");
		counter++;
		b.append(tab + "" + seg[count].getPoint(1).getCoord(0) + " "
				+ seg[count].getPoint(1).getCoord(1) + " "
				+ seg[count].getPoint(1).getCoord(2) + "  #Index" + counter
				+ "\n");
	...
	private static String writeMbb(MBB3D mbb, double[] rgb) {
		...
		for (int i = 0; i < 7; i++) {
			b.append(tab + "" + pts[i].getCoord(0) + " " + pts[i].getCoord(1)
					+ " " + pts[i].getCoord(2) + ",  #Index" + i + "\n");
		}
		b.append(tab + "" + pts[7].getCoord(0) + " " + pts[7].getCoord(1) + " "
				+ pts[7].getCoord(2) + ",  #Index7\n");
	...
	private static String writeVector(Vector3D vec, Point3D from, double[] rgb) {
		...
		temp.normalize(new GeoEpsilon(TestConstants.EPSILON));
		...
		// segment of vector
		Segment3D seg = new Segment3D(from, to, new GeoEpsilon(
				TestConstants.EPSILON));
		b.append("Transform {\n children [\n  Shape {\n  appearance Appearance {\n    material Material { }\n  }\n");
		b.append(tab + "geometry IndexedLineSet { \n" + tab
				+ "coord Coordinate { \n" + tab + "point [ \n");
		...
		// arrow
		// Rotationsberechnung
		Vector3D yVec = new Vector3D(0, 1, 0); // steht fuer ausgangsrichtung
												// des Kegels
		Vector3D normDirection = vec.copy();
		normDirection.normalize(new GeoEpsilon(TestConstants.EPSILON));
	...
	private static String writePlane(Plane3D plane, double[] rgb) {
		...
		b.append("\nTransform {\n children [\n  Shape {\n  appearance Appearance {\n    material Material {emissiveColor 0 0.5 0.3   diffuseColor "
				+ rgb[0]
				+ " "
				+ rgb[1]
				+ " "
	 ...
	 * @return String - VRML-Group element
	 */
	private static String writeTetraNet(Tetrahedron3DNet net, double[] rgb) {
		StringBuffer b = new StringBuffer();
		...
		for (int i = 0; i < net.countComponents(); i++) {
			...
			Tetrahedron3DComponent tetComp = net.getComponent(i);
